      ...
      <main className="w-full max-w-md px-6 pt-24 pb-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2" style={{ color: cat.cor }}>
            ...
            <span className="font-headline text-[12px] font-bold tracking-widest uppercase">
              {data.modulo}
            </span>
          </div>
          <h2 className="font-headline text-[24px] leading-[32px] font-bold text-[#151e12]">
            {data.titulo}
          </h2>
        </div>
        ...
        {/* Main Info Card */}
        <div
          className={`relative overflow-hidden rounded-xl bg-white border-2 p-6 flex flex-col gap-4 ${cat.glowCard}`}
          style={{ borderColor: `${cat.cor}33` }}
        >
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <span className="material-symbols-outlined text-[96px]" style={{ color: cat.cor }}>
              {cat.icon}
            </span>
          </div>
          ...
          <div className="space-y-4">
            <h3
              className="font-headline text-[18px] leading-[28px] font-bold"
              style={{ color: cat.cor }}
            >
          ...
          <div className="grid grid-cols-2 gap-3 mt-2">
            {data.stats.map((stat, idx) => (
              <div
                key={idx}
                className="bg-[#e7f1dd] p-4 rounded-lg flex flex-col gap-1 border border-[#bccbb2]"
              >
                <span className="font-bold text-2xl" style={{ color: cat.cor }}>
                  {stat.valor}
                </span>
                <span className="text-[10px] font-headline font-bold text-[#3d4b37] uppercase tracking-tighter leading-tight">
                  {stat.label}
                </span>
              </div>
            ))}
          ...
          {data.cards.map((card, idx) => (
            ...
            >
              <div className="p-2 rounded-lg shrink-0" style={{ backgroundColor: `${cat.cor}1A` }}>
                <span className="material-symbols-outlined" style={{ color: cat.cor }}>
                  {card.icone}
                </span>
              </div>
              <div>
                <h4 className="font-headline text-[16px] leading-[20px] font-bold mb-1">
                  {card.titulo}
                </h4>
                <p className="text-sm text-[#3d4b37] font-body text-[14px] leading-[20px]">
                  {card.texto}
                </p>
              </div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl scale-100 transition-transform duration-300">
            ...
            >
              <span className="material-symbols-outlined text-4xl" style={{ color: cat.cor }}>
                menu_book
              </span>
            </div>
            ...
            <p className="text-[#3d4b37] font-body text-[16px] leading-[24px] mb-8">
              Preparamos um guia rápido com contatos de emergência e canais oficiais para denúncias
              seguras.
            </p>
            <button
              onClick={() => setShowModal(false)}
